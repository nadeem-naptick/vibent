...
function ensureCssImport(files: Array<{ path: string; content: string }>): Array<{ path: string; content: string }> {
  ...
  if (mainJsxIndex !== -1 && indexCssExists) {
    ...
    if (!hasIndexCssImport) {
      console.log('[apply-ai-code-stream] ⚠️ main.jsx missing index.css import - auto-fixing...');
      ...
      if (insertIndex !== -1) {
        ...
        console.log('[apply-ai-code-stream] ✅ Added missing index.css import to main.jsx after line', insertIndex + 1);
      } else {
        // Fallback: add at the beginning
        files[mainJsxIndex].content = "import './index.css'\n" + content;
        console.log('[apply-ai-code-stream] ✅ Added index.css import at the beginning of main.jsx');
      }
    } else {
      console.log('[apply-ai-code-stream] ✅ main.jsx already has index.css import');
    }
  }
